...
export const Header=({sounder}:{sounder:({id}:{id:string})=>void})=>{
    ...
    return (
        <div className="bg-sky-500 h-[10%] flex items-center justify-between relative px-1">
           <div><img src="/logo192.png" className="ml-2 h-12 w-12 sm:h-18 sm:w-18 top-2 left-1 rounded-full" alt="" /></div>
           ...
           <div className="absolute bottom-[3%] left-[50%] -translate-x-[50%] text-white texl-md md:text-3xl font-sans text-center font-black">{state.userName}</div>
       
       <div><img src="/kelly.png" alt="kelly" className="mr-2 h-12 w-12 sm:h-18 sm:w-18 top-2 right-1 rounded-full" /></div>
        </div>
    )
}